  ...
  return {
    ...
    uXTZ: {
      ...
      market: {
        ...
        configuration: {
          interestRateModel: {
            address: (env.UXTZ_IR_ADDRESS || undefined) as ContractAddress,
            kinkF: "500000000000000000", // 50%
            baseRateF: "0", // 0%
            multiplierF: "2487494954", // 7,84%/year
            jumpMultiplierF: "85597887878", // 270%/year
            reserveFactorF: "0",
          } as IRModel,
          token_metadata: MichelsonMap.fromLiteral({
            symbol: Buffer.from("y-uXTZ").toString("hex"),
            name: Buffer.from("Yupana uXTZ").toString("hex"),
            decimals: Buffer.from("12").toString("hex"),
            is_transferable: Buffer.from("true").toString("hex"),
            is_boolean_amount: Buffer.from("false").toString("hex"),
            should_prefer_symbol: Buffer.from("false").toString("hex"),
            thumbnailUri: Buffer.from(
              "ipfs://QmVL6fBrPYXUfsndkgndZQdNh8KPeT2Ls2up8HxiWqYWwf"
            ).toString("hex"),
          }) as MichelsonMap<string, BytesString>,
          asset: {
            fA2: {
